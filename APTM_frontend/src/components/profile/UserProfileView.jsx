...
import { 
  FaComments, FaChartLine, FaNewspaper, FaArrowLeft, FaCamera, FaEdit,
  FaMapMarkerAlt, FaCalendarAlt, FaBriefcase, FaTwitter, FaLinkedin,
  FaGithub, FaGlobe, FaWhatsapp, FaFacebook, FaDiscord, FaTelegram,
  FaReddit, FaYoutube, FaInstagram, FaHeart, FaRegHeart, FaBookmark,
  FaRegBookmark, FaHome, FaRetweet, FaWifi, FaExclamationCircle, FaPlug,
  FaSignal, FaChevronLeft, FaChevronRight, FaUserPlus, FaUserCheck,
  FaComments as FaMessage, FaChartBar, FaTrophy, FaStar
} from 'react-icons/fa';
...
const UserProfileView = () => {
  ...
  const [hasTradingConnection, setHasTradingConnection] = useState(false);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('panelStateChange', handlePanelStateChange);
  }, []);
  ...
  useEffect(() => {
    ...
    userStatusService.init(currentUser, {
      onConnect: () => {
        ...
        if (profileUser?.id) {
          setTimeout(() => userStatusService.getUserStatus(profileUser.id), 500);
        }
      },
      ...
      onUserStatusResponse: (data) => {
        if (data.userId === profileUser?.id) {
          ...
          if (!data.isOnline && data.lastSeen) setLastSeen(data.lastSeen);
        }
      }
    });

    return () => userStatusService.disconnect();
  ...
  const fetchTradingAnalytics = useCallback(async () => {
    ...
    try {
      ...
      if (isOwnProfile) {
        // Fetch full analytics for own profile
        ...
        if (response.ok) {
          ...
          if (result.success && result.data) {
            ...
            setUserBadge(result.data.badge);
            setExperienceLevel(result.data.experienceLevel);
            setHasTradingConnection(result.data.hasConnections);
            setConnectedPlatforms(result.data.connectedPlatforms || []);
          }
        }
      } else {
        // Fetch public badge for other profiles
        const response = await fetch(`${API_URL}/profile/public/badge/${targetUserId}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        
        if (response.ok) {
          const result = await response.json();
          if (result.success && result.data && !result.data.isPrivate) {
            setUserBadge(result.data.badge);
            setTradingStats({
  ...
  const formatUserData = useCallback((userData, isPublic = false) => {
    ...
    let user = userData;
    if (userData.profile && !userData.id && !userData._id) user = { ...userData, ...userData.profile };
    if (userData.user) user = { ...userData.user, ...userData };
    ...
    if (avatarData && typeof avatarData === 'object') avatarData = avatarData.url || avatarData.avatarUrl || null;
    ...
    if (bannerData && typeof bannerData === 'object') bannerData = bannerData.url || bannerData.bannerUrl || null;
    ...
    return {
      ...
      tradingConnections: user.tradingConnections,
      profile: {
        followers: followers,
        following: following,
        postsCount: postsCount,
      ...
      if (isOwnProfile) {
        ...
        if (response.ok) {
          ...
          if (result.success && (result.user || result.profile)) {
            ...
            if (result.user?.badge) setUserBadge(result.user.badge);
            if (result.user?.tradingStats) setTradingStats(result.user.tradingStats);
            if (result.user?.tradingConnections) {
              setHasTradingConnection(
                result.user.tradingConnections?.mt4?.connected || 
                result.user.tradingConnections?.mt5?.connected
              );
            }
          }
        }
  ...
  }, [targetUserId, isOwnProfile, currentUser, formatUserData, fetchTradingAnalytics]);
  ...
  const fetchUserPosts = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/posts/user/${profileUser.id}`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const fetchUserGallery = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/gallery/${profileUser.id}`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      if (response.ok) {
        const result = await response.json();
  ...
  const handleTradingStatsUpdate = useCallback((data) => {
    if (data) {
  ...
  const renderTradingTab = () => (
    <div className={styles.tradingTab}>
      {isOwnProfile ? (
        <TradingConnection 
          onStatsUpdate={handleTradingStatsUpdate}
        />
      ) : (
        <div className={styles.publicTradingStats}>
          <h3>Trading Performance</h3>
          {tradingStats && tradingStats.totalTrades > 0 ? (
            <div className={styles.publicStatsGrid}>
              <div className={styles.publicStat}>
                <span className={styles.publicStatValue}>{tradingStats.totalTrades}</span>
                <span className={styles.publicStatLabel}>Total Trades</span>
              </div>
              <div className={styles.publicStat}>
                <span className={styles.publicStatValue}>{tradingStats.winRate}%</span>
                <span className={styles.publicStatLabel}>Win Rate</span>
              </div>
              {userBadge && (
                <div className={styles.publicBadge}>
                  <span className={styles.publicBadgeIcon}>{userBadge.icon}</span>
                  <span className={styles.publicBadgeTitle}>{userBadge.title}</span>
                </div>
              )}
            </div>
          ) : (
  ...
  const renderTabContent = () => {
    switch(activeTab) {
      ...
      case 'overview': return <OverviewTab profileUser={profileUser} posts={posts} onStatClick={handleStatClick} tradingStats={tradingStats} badge={userBadge} />;
